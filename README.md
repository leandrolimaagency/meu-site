...
- `VSL_INICIAL_SRC` — URL do MP4 da VSL inicial. O arquivo mora no Vercel Blob
  (store `funil-stalk-v2-media`), não no deploy: com 96MB ele travava o upload da CLI.
  Para trocar o vídeo, suba a nova versão com `vercel blob put <arquivo> --access public`
  e cole a URL retornada aqui. A cópia local em `assets/video/` fica fora do deploy
  (ver `.vercelignore`) e serve só para rodar localmente.
